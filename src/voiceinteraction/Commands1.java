 ...
 * ends, an instruction is sent according to number.
 * Every recognized message is treated by a specific private function.
 * 
...
public class Commands1 {
    ...
    private static final ArrayList<String> designateFormNames = new ArrayList();
        ...
        if (findWord(strings, "Palette:MouseReleased")) {
            ...
            if (create && ici) {
                ...
                if (ici && !(names.isEmpty())) {
                    ...
                    System.out.println("DEBUG > deplacement de " + "x=" + deltaX + " y=" + deltaY);
                    ...
                    try {
                        ...
                        Logger.getLogger(Commands1.class.getName()).log(Level.SEVERE, null, ex);
                    }
    ...
    private static void color(String[] strings, Ivy bus) {
        if (findWord(strings, "Couleur")) {
            ...
            if (create) {
                ...
                // PROBLEM ICI : ELEMENT[3] et/ou designateColorName sont nuls
                /*names.stream().filter((element) -> (element[3] == null)).forEach((element) -> {
                 System.out.println("DEBUG > element is null");
                 });*/
                names.stream().filter((element) -> (element[3].equals("none"))).forEach((element) -> {
                    System.out.println("DEBUG > color is " + element[3]);
                });
                System.out.println("DEBUG > designateColorName is " + designateColorName);
                names.stream().filter((element) -> (element[3].equals("none")
                        || !element[3].contains(designateColorName))).forEach((element) -> {
