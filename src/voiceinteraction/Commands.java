...
public class Commands {
    ...
    private static boolean waitingForShowedColor = false;
    ...
    public static void commandsTreatment(String[] strings, Ivy bus) {
        ...
        // Get a webcam recognized pattern
        webcam(strings, bus);
    }
    ...
    private static void initialiser(String[] strings) {
        if (findWord(strings, "initialiser")) {
            ...
            if (!isThereAPaletteFrame && findWord(strings, "palette")) {
                PaletteGraphique myPG = new PaletteGraphique(address, 0, 0,
                        300, 300);
            ...
            if (!isThereAHergerFrame && findWord(strings, "herger")) {
                recogestureherger.HergerUI myHerger = new recogestureherger.HergerUI(
                        address, 0, 0, 300, 300);
            ...
            if (create && here) {
                ...
                if (here && !(names.isEmpty())) {
                    ...
                    deltaX = Integer.parseInt(x)
                            - Integer.parseInt(names.get(0)[1]);
                    deltaY = Integer.parseInt(y)
                            - Integer.parseInt(names.get(0)[2]);
                    ...
                    try {
                        ...
                        Logger.getLogger(Commands.class.getName()).log(
    ...
    private static void color(String[] strings, Ivy bus) {
        if (findWord(strings, "Couleur")) {
            ...
            // Remember color
            if (create) {
                if (findWord(strings, "de cette couleur")
                        && !waitingForShowedColor) {
                    waitingForShowedColor = true;
                } else {
                ...
                if (findWord(strings, "de cette couleur")
                    ...
                    if (findWord(strings, "red")) {
                        designateColorName = "rouge";
                    } else if (findWord(strings, "green")) {
                        designateColorName = "vert";
                    } else if (findWord(strings, "blue")) {
                        designateColorName = "bleu";
                    }

                    // We remove all forms which are not of specified color
                    // Forced to use a temp because we can't remove an element
                    // in the loop
                    ArrayList<String[]> temp = new ArrayList<>();
                    names.stream().filter((element) -> (element[3].contains(designateColorName)))
                            .forEach((element) -> {
                                temp.add(element);
                            });
                    names = temp;
    ...
    private static void resultAskingInfo(String[] strings) {
        if (findWord(strings, "Palette:Info")) {
            if (moving) {
                ...
                // We add all information inside names, to get what we want
                // later
             ...
             * names.stream().map((element) -> { for (String string : element) {
             * System.out.print(string + "-"); } return element;
             * }).forEach((_item) -> { System.out.print("|"); });
             * System.out.println();
             */
        }
    }
    ...
    private static void webcam(String[] strings, Ivy bus) {
        if (findWord(strings, "Webcam") && waitingForShowedColor) {
            String[] infoPatternShowed = strings[0].split("marqueur=");
            if (create) {
                switch (infoPatternShowed[1]) {
                    case "0":
                        colorName = "couleurFond=255:0:0 couleurContour=255:0:0";
                        break;
                    case "1":
    ...
    private static void executeCommands(Ivy bus) {
        try {
            switch (state) {
                ...
                case 5:
                    for (String form : designateFormNames) {
                        bus.sendMsg("Palette:DeplacerObjet nom=" + form + " "
                                + movement);
                    }
                    break;
                default:
            ...
            Logger.getLogger(Commands.class.getName()).log(Level.SEVERE, null,
                    ex);
        }
